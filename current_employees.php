<?php
	...
	//get number of rows
	$queryNum = $db_connect->query("SELECT COUNT(*) as postNum FROM sharp_emp WHERE status = 'current'");
...
?>
<!-- Conditionally load Font Awesome CSS based on internet connection status -->
<script>
  function loadFontAwesome() {
    ...
    if (online) {
      // Load Font Awesome from CDN when online
	  ...
	  <a href="https://www.linkedin.com/company/sukkur-iba-university/?originalSubdomain=pk" target="_blank" class="linkedin-icon"><i class="fab fa-linkedin"></i></a> <!-- Replace YOUR_LINKEDIN_PROFILE_URL_HERE with your LinkedIn profile URL -->
		...
		<div class="container">
			...
			<div class="wrapper employee_list clearfix">
				...
				<div class="section_title">The Status Of All Current Employees Record</div>
				<div class="top-bar">
					<div class="top-item">
					...
					<div class="top-item">
						<a href="downloadcurrent.php" class="download-button">Download Current Record </a>
           </div>
				</div>
				<?php
					$getemp = mysqli_query($db_connect, "SELECT * FROM sharp_emp WHERE status = 'current' ORDER BY id ASC");
